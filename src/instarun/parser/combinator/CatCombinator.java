...
public final class CatCombinator extends CombinatorWithManyParsers {
    ...
    public void parse(final int index, final @NotNull InstaTramp tramp) {
        ...
        Gll.pushListener(
                tramp, new InstaNodeKey(index, parsers.getFirst()),
                GllParserListeners.catListener(AutoFlattenSeq.make(), parsers.subList(1, parsers.size()), new InstaNodeKey(index, this), tramp));
    }
    ...
    public void fullParse(final int index, final @NotNull InstaTramp tramp) {
        ...
        Gll.pushListener(
                tramp, new InstaNodeKey(index, parsers.getFirst()),
                GllParserListeners.catFullListener(AutoFlattenSeq.make(), parsers.subList(1, parsers.size()), new InstaNodeKey(index, this), tramp));
    }
